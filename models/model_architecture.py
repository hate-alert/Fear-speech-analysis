...
def select_transformer_model(type_of_model,path,params,weights=None):
...
class DocumentBERTLSTM(BertPreTrainedModel):
    ...
    def forward(self, document_batch, document_sequence_lengths,labels= None,device='cuda'):
...
#         self.weights=params['weights']
    ...
    def forward(self, document_batch, document_sequence_lengths,labels= None,device='cuda'):
        ...
        for doc_id in range(document_batch.shape[0]):
            bert_output[doc_id][:self.bert_batch_size] = self.dropout(self.roberta(document_batch[doc_id][:self.bert_batch_size,0],
                                            token_type_ids=document_batch[doc_id][:self.bert_batch_size,1],
                                            attention_mask=document_batch[doc_id][:self.bert_batch_size,2])[1])
